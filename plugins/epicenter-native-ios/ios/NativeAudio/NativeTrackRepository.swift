...
final class NativeTrackRepository {
    ...
    /// Typed library listing used by native surfaces (e.g. CarPlay) that need model
    /// objects rather than the JSON dictionary shape the Capacitor bridge consumes.
    func loadTracks(offset: Int = 0, limit: Int = 500, sort: String? = "addedAt") -> [NativeTrack] {
        database.getLibraryPage(offset: offset, limit: limit, search: nil, sort: sort).tracks
    }
